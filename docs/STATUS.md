...
### C88. 3,800 files offered by name that their decoder reads as another format — **found 2026-09-23; FIXED the same day on `fix/c88-refused-directories`, not yet seen on the phone**
...
the next start with no download. SidMon 1 is left alone. The recompute's SQL now lives in one
