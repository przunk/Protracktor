#!/usr/bin/env python3
"""Measure whether ayfly plays the ZX Spectrum formats this build cannot.
...
The same two questions the other probes ask, from `Backend`'s contract rather than from hope:
  ...
  does it end     `ay_getsonglength` -- if the library states one, these files arrive with a seek
                  bar the way AHX did, and nothing has to be stored to give them one
...
Usage:  ./scripts/probe-ayfly.py [--files N] [--seed N]
"""
...
PROBE = ROOT / "native" / "probe" / "ayfly" / "build" / "probe-ayfly"

# What ayfly's `players/` covers, ordered by how much of Modland's Spectrum tree each is worth.
EXTENSIONS = ["pt3", "pt2", "stc", "asc", "sqt", "stp", "psc", "psg", "vtx", "ay"]
...
def main() -> int:
    ...
    if not PROBE.is_file():
        print(f"❌ {PROBE.name} is not built. Run ./scripts/build-ayfly-probe.sh first.")
        return 1
    ...
    print("Modland holds, of the formats ayfly claims:\n")
    ...
    (CACHE / "ayfly.json").write_text(json.dumps(
